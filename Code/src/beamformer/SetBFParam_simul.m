...
%
%  2019-06-29
%  use a separate SetBFParam for simulation (for zero nSysOffset_sec)
%
%  2019-05-22
%  IQ/RF beamformer for plane wave imaging
%  linear
%
function stBFpm = SetBFParam_simul(sBfType, stRFInfo, nMultiBeam, nQiq, nQph, sBfOutName)
    
    stTrans = stRFInfo.stTrans;
    assert(strcmp(stRFInfo.sVersion,'2.0')||strcmp(stRFInfo.sVersion,'2.1')||strcmp(stRFInfo.sVersion,'2.6'),'version mismatch'); % nRcvChannel should be included
    
    
    stBFpm.sVersion = '3.0';
    stBFpm.bPlot = 1;
    stBFpm.bSaveEachAngle = 0; 
    stBFpm.sBfType = sBfType;
